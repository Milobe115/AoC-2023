...
def calculate_beam_score(lines, start_x, start_y, start_dir):
...
def main():
    ...
    for i in range(len(lines)):
        score = max(score, calculate_beam_score(lines, 0, i, Direction.EAST))
        score = max(score, calculate_beam_score(lines, len(lines[0]) - 1, i, Direction.WEST))
    for i in range(len(lines[0])):
        score = max(score, calculate_beam_score(lines, i, 0, Direction.SOUTH))
        score = max(score, calculate_beam_score(lines, i, len(lines) - 1, Direction.NORTH))
